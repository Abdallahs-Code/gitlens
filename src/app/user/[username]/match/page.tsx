...
export default function MatchPage() {
  ...
  const handleAnalyzeMatch = async () => {
    ...
    try {
      ...
      try {
        [jobResult, profileResult] = await Promise.all([
        ...
        if (code === 429) {
          setError(`${message} Cannot perform gemini's initial preprocessing. Try again later or get a new `);
        } else if (code === 401) {
          setError(`${message} Cannot perform initial preprocessing. Get a new `);
        } else if (code === 400) {
          setError(`${message} Cannot perform initial preprocessing. Set a `);
        } else {
          setError(message);
        }
        return;
              ...
              <div className="mb-6 p-4 rounded-lg" style={{ border: "1px solid var(--color-background-light)", background: "var(--color-background-dark)" }}>
                <p className="text-text-muted text-sm italic">
                  {error === "llm" ? (
                    ...
                    <>
                      {error}
                      {(error.endsWith('get a new ') || error.endsWith('Get a new ') || error.endsWith('Set a ')) && (
